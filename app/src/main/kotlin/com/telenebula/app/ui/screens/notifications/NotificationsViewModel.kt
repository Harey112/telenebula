...
class NotificationsViewModel(private val prefs: PrefsRepository, private val openWith: OpenWith, private val navigator: Navigator) : ViewModel() {
    ...
    val state: StateFlow<NotificationsUiState> = combine(prefs.prefs, menus.key) { p, key -> NotificationsUiState(p.core.notifications, key) }
        .uiState(viewModelScope, NotificationsUiState(prefs.prefs.value.core.notifications))
    ...
    fun resetToDefaults() = prefs.update { it.copy(core = it.core.copy(notifications = NotificationPrefs())) }
